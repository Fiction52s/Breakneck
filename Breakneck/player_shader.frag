...
void main() {
	...
	//gl_FragColor = LightColor* max(dot(N, L), 0.0);
	//gl_FragColor = DiffuseColor * LightColor * max(dot(N, L), 0.0);//* vec4(Intensity,1) * max(dot(N, L), 0.0);// vec4(FinalColor, DiffuseColor.a);
}